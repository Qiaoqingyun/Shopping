            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                <li :class="{ active: isOne }" @click="changeOrder('1')">
                  <a>综合<span v-if="isOne" class="iconfont"
                      :class="{ 'icon-xiangshangjiantou': isAsc, 'icon-xiangxiajiantou': isDesc }"></span></a>
                </li>
                <li :class="{ active: isTwo }" @click="changeOrder('2')">
                  <a>价格<span v-if="isTwo" class="iconfont"
                      :class="{ 'icon-xiangshangjiantou': isAsc, 'icon-xiangxiajiantou': isDesc }"></span> </a>
                </li>
              </ul>
            </div>
...
export default {
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      this.getData()
      //url栏也需要修改:进行路由跳转
      //本意是删除query参数，如果路径中有params参数不应该删除，应该带走
    ...
    //排序的操作
    changeOrder(flag) {
      // flag形参是一个标记 代表用户点击的是综合(1)还是价格(2)
      // 点击时传过来
      let originOrder = this.searchParams.order //最开始的状态
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      //再次发起请求
      this.getData()
      //每次请求完毕，应该把1,2,3级分类的id置空，让他接受下一次相应的id
      this.searchParams.category1Id = undefined
      this.searchParams.category2Id = undefined
      this.searchParams.category3Id = undefined
    }
